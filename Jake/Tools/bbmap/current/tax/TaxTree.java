...
//					failed++;
	...
	public boolean isPlant(int taxID) {
		...
		while(tn.id!=tn.pid){
			if(tn.id==VIRIDIPLANTAE_ID){return true;}
			tn=getNode(tn.pid);
		}
		return false;
	}
	...
	public boolean isAnimal(int taxID) {
		TaxNode tn=getNode(taxID);
		while(tn.id!=tn.pid){
			if(tn.id==METAZOA_ID){return true;}
			tn=getNode(tn.pid);
		}
		return false;
	}
	...
	public boolean isEukaryote(int taxID) {
		...
		while(tn.id!=tn.pid){
			if(tn.id==EUKARYOTA_ID){return true;}
			tn=getNode(tn.pid);
		}
		return false;
	}
	...
	public static int parseHeaderStatic(String header){
		...
		int id=-1;
		try {
			id=Tools.parseInt(header, idx+1, idx2);
...
//		return levelExtended!=NO_RANK_E && (levelExtended==levelToExtended(level)/* || levelExtended==STRAIN_E*/);
	...
	public static final int numTaxaNames=taxaNames.length;
	...
	public static final int numTaxaNamesExtended=taxaNamesExtended.length;
