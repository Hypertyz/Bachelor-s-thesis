...
//		System.err.println("\n"+header+"\n"+slash1+","+slash2);
		zmwID=Tools.parseInt(header, slash1+1, slash2);
		start=Tools.parseInt(header, slash2+1, under3);
		stop=Tools.parseInt(header, under3+1, terminator);
	}
	...
	public static int parseZMW(final String header){
		...
		int slash2=header.indexOf('/', slash1+1);
		assert(slash2>=0) : "Misformatted PBHeader: "+header;
		int zmwID=Tools.parseInt(header, slash1+1, slash2);
		return zmwID;
	}
